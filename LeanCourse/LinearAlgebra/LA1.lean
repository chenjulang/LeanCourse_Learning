...
-- variable (f : m → n → α)
-- #check of.symm -- Matrix ?m.5656 ?m.5657 ?m.5658 ≃ (?m.5656 → ?m.5657 → ?m.5658)
-- #check of.symm f -- (fun x ↦ m → n → α) f
-- #check of.symm f _ _
...
* `AlgEquiv.mapMatrix`
-/
def map (M : Matrix m n α) (f : α → β) : Matrix m n β := by
  have h1 := fun i j => f (M i j)
  have h2 := (of h1)
  exact h2
...
@[simp]
theorem transpose_apply (M : Matrix m n α) (i j)
...
def conjTranspose [Star α] (M : Matrix m n α)
: Matrix n m α :=
  M.transpose.map star
#align matrix.conj_transpose Matrix.conjTranspose

def conjTranspose2 [Star α] (M : Matrix m n α) : Matrix n m α :=
  --  M.transpose.map (@star _ _) --第二种写法
  M.transpose.map (λ x ↦ Star.star x)

-- 让我们逐步解释这个定义：

-- conjTranspose是函数的名称，表示对矩阵进行共轭转置操作。
-- [Star α]是一个类型类约束，它要求类型α具有一个名为Star的类型类实例。这意味着在这个定义中，α必须具有共轭操作。
-- (M : Matrix m n α)是函数的参数，表示输入的矩阵M，其大小为m行n列，并且元素类型为α。
-- : Matrix n m α是函数的返回类型，表示转置后的矩阵，大小为n行m列，并且元素类型仍为α。
-- 函数体部分：

-- M.transpose表示将矩阵M进行转置操作，即行变为列，列变为行。
-- .map star表示应用一个叫做star的映射函数到转置后的矩阵上。这个映射函数的作用是将矩阵中的每个元素应用共轭操作。
-- 因此，整体来看，conjTranspose函数接受一个矩阵作为输入，首先对该矩阵进行转置操作，然后对转置后的矩阵中的每个元素应用共轭操作，最后返回结果矩阵，其大小为转置前的列数行数，并且元素类型与输入矩阵相同。

-- 在这个定义中，star是一个函数或操作符的名称，并不是凭空出现的。它被引入的方式是通过类型类约束 [Star α]，其中 Star 是一个类型类，要求类型 α 具有对应的实例。

-- 类型类是一种用于表示某些类型所需的公共接口的机制。[Star α]的作用是告诉编译器，在执行 conjTranspose 函数时，我们需要确保类型 α 具有一个名为 Star 的类型类实例。这样，函数体内的 .map star 就可以成功调用名为 star 的函数或操作符。

-- 因此，在使用 conjTranspose 函数之前，你需要确保在相应的上下文中，类型 α 已经具有 Star 类型类的实例，以便 star 函数能够被调用。
...
-- col是函数的名称。
-- (w : m → α)是函数的参数，其中 w 是一个接受类型为 m 的参数并返回类型为 α 的结果的函数。换句话说，w 是一个从索引 m 到元素 α 的映射。
-- : Matrix m Unit α是函数的返回类型，表示一个大小为 m 行、单列（即只有一列）的矩阵，其元素类型为 α。
-- 函数体部分：

-- of fun x _ => w x 创建了一个函数对象，它接受两个参数 x 和 _，并返回 w x。在这里，_ 表示一个不被使用的占位符，因为矩阵的列数为 Unit 类型，它只有一个取值 ()，不需要使用具体的列索引。
-- 通过 of 函数，将上述匿名函数封装成一个矩阵对象，并作为结果返回。
-- 因此，整体来看，col 函数接受一个从索引 m 到元素 α 的映射 w，然后创建一个只有一列的大小为 m 行、单列的矩阵，该矩阵的每个元素都由映射 w 决定。换句话说，col 函数用于构造一个以给定映射为列的矩阵对象。

--------

-- 要从类型为 Matrix m Unit α 的矩阵中取出某一项，可以使用索引 (i, ())，其中 i 是行索引。由于矩阵的列类型为 Unit，只有一个取值 ()，因此我们可以使用这个索引来表示矩阵中的某个元素。

-- 以下是一个示例，展示如何从 Matrix m Unit α 类型的矩阵中取出特定项：

-- def getMatrixItem (M : Matrix m Unit α) (i : m) : α :=
--   M i ()
-- 在这个例子中：

-- (M : Matrix m Unit α) 是函数的第一个参数，表示输入的矩阵，类型为 Matrix m Unit α。
-- (i : m) 是函数的第二个参数，表示要获取的元素所在的行索引。
-- : α 是函数的返回类型，表示获取到的元素类型为 α。
-- 函数体部分使用 M i ()，即将行索引 i 和列索引 () 传递给矩阵 M，从而获取矩阵中对应位置的元素。

-- 通过这种方式，我们可以从类型为 Matrix m Unit α 的矩阵中准确地取出指定位置的元素。请注意，由于矩阵的列类型为 Unit，因此不需要提供具体的列索引，而是使用 () 表示唯一的列索引。

-- TODO: set as an equation lemma for `col`, see mathlib4#3024
@[simp]
theorem col_apply (w : m → α) (i j)
: col w i j = w i
:=
  rfl
...
@[simp]
theorem row_apply (v : n → α) (i j)
: row v i j = v j :=
  rfl
#align matrix.row_apply Matrix.row_apply

instance inhabited [Inhabited α]
...
instance decidableEq [DecidableEq α] [Fintype m] [Fintype n]
