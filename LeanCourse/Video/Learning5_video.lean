...
--  2.1.cyclotomic n R：n-分圆多项式，z^n=1,(x-x1)(x-x2)(x-x3)...(x-xn)，系数在R集合里，即
        -- 2.1.1.为什么：复数单位根是z = e^((2πik) / n) (k=1,2,3...n)
          ...
          -- 现在我们考虑复数单位根，即满足zⁿ = 1的复数。假设z可以表示为z = Ce^(iθ)，C是实数，可以表示的原因是：z是一个任意的复数(Ccos(θ) + iCsin(θ))=Ce^(iθ)
          ...
          -- (C^n)*cos(nθ) = 1
          -- (C^n)sin(nθ)  = 0 -- 小引理：常数(C^n)值只能是+1or-1? 因为满足第二行的nθ只能是πk，即π的整数倍，代入第一行之后，
          -- 就是(C^n)*cos(kπ)= 1 , (C^n)*(1或-1) = 1 ，C是实数
