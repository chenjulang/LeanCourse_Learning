  ...
  --  2.1.cyclotomic n R：n-分圆多项式，z^n=1,(x-x1)(x-x2)(x-x3)...(x-xn)，系数在R集合里，即 为什么是n个单位根呢？下面会讲：
          ...
          -- 下面讨论分圆多项式
            ...
            -- “递进式证明”，可以证明起来舒服一点～～～，而不是直接一般化

            -- 分圆多项式，只是一个构造定义，良定义，指所有能生成全体复数根的复数根x1,x2,...xm , 然后拼成多项式(x-x1)(x-x2)...(x-xm)：
            -- z = e^(i2k₂π/n) k₂=1,2,3,4... 中k=1的项必定为本原根。
            -- n=1 , 很明显只有一个复数根，本原根也只有一个。
              --（x-e^(i(2π) / 1)）= x-1
            -- n=2 ，除了k=1,就没有了。
            ...
            -- n=3, 除了k=1, 还有k=2,因为所有根为 2/3 4/3 6/3=0 ， 所以4/3 通过整数倍可以生成其他的2*4/3=8/3=[2/3] 3*4/3=12/3=[6/3]=0 16/3=[4/3] [8/3]每次分子加4
             ...
             -- n=4 ，除了k=1,还有k=3 , （其实这里已经发现规律，满足的k，k都是和n互质的）
              -- （x-e^(i(1*2π)/4)）* （x-e^(i(3*2π)/4)））=
              -- x^2系数: 1
              -- x^1系数: -e^(i(1*2π)/4) -e^(i(3*2π)/4)）= - （0 刚好是0吗??? + 0 刚好是0吗???） = 0
              -- x^0系数: 2/4 + 6/4 = 8/4 = 2 刚好是2的整数倍???试下其他偶数
            ...
            -- n=4
              -- x^4-1 = d|4 的所有d分圆多项式连乘,此处数量是4的因子的个数 = 1分圆 * 2分圆 * 4分圆
              --

            -- 引理：x^3-1 = 所有复数根xk组成的单项式(x-xm)连乘得到的多项式，此处数量是n个 = (x-x1)(x-x2)...(x-xn)
            -- 其实就是将所有根写成多项式
            -- 举例：
            -- （x-e^(i(1*2π)/3)）* （x-e^(i(2*2π)/3)）） * （x-e^(i(3*2π)/3)））=？ 是否等于x^3-1
            -- 也就是k=1,2,3相乘，之前计算的结果相当于说，k=1,2的结果A1，（x-1）是A2， x^3-1是等于A1*A2的
            -- 我们发现k=1,2,3相乘 对比 A1*A2 ， 刚好其中抵消了A1， 还剩k=3和A2对比，k=3必定是1，因为是一个周期，所以是x-1， 和固定A2必定相等。



            -- 后面单独讲：我们试一下n=4,x^4 - 1 = （x-e^(i(1*2π) / 4)）*（x-e^(i(2*2π) / 4)）* （x-e^(i(3*2π) / 4)） * （x-e^(i(4*2π) / 4)）
            -- x^4 - 1(这个1只要π的整数倍即得1，不需要2π得整数倍)
            -- 抽象写法 x^1* （(2+3+4)+(1+3+4)+(1+2+4)+(1+2+3)）

            -- 关于x轴对称的一对对，对称的奇妙之处，后面讲一下???n=5的情况
            -- x^5 - 1 = （x-e^(i(1*2π)/5) *（x-e^(i(2*2π)/5) *（x-e^(i(3*2π)/5)）* （x-e^(i(4*2π)/ 5)） * （x-e^(i(5*2π)/ 5)）
            -- x^0 系数为-1，也就是这里乘积为1: 抽象写法去掉iπ外的项：2/5 + 4/5 + 6/5 + 8/5 + 10/5 = （1*2+5*2）*5/2 /5 只要是偶数即可= 6π
            -- x^1 系数为0:
            -- x^2 系数为0:
            -- x^3 系数为0:
            -- x^4 系数为0:
            ...
            -- 后面讲一下???n=6的情况
            -- x^6 - 1 = （x-e^(i(1*2π)/6) *（x-e^(i(2*2π)/6)） *（x-e^(i(3*2π)/6)）* （x-e^(i(4*2π)/6)） * （x-e^(i(5*2π)/6) * （x-e^(i(6*2π)/6)
