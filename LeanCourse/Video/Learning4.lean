...
noncomputable section
  ...
  section Module
    ...
    -- 代入某个d以后，剩下的 d.sum (fun i => F i) 具体做的是：
    -- 展开d.sum: ∑ a in d.support, g a (d a)
    -- 代入g，g就是(fun i => F i)：∑ a in d.support, (fun i => F i) a (d a)
    -- a是什么，先看d.support是指d中有非零元素的索引位置的集合，即{1,2},因为(7，8)的第1，2位都是非零数
    -- 所以展开a就是
    -- (fun i => F i) 1 (d 1) +
    -- (fun i => F i) 2 (d 2)
    -- 代入(fun i => F i）得到,并且代入变量d：(F 1) (7) + (F 2) (8)
    -- F是什么呢？根据total定义知道是LinearMap.id.smulRight (v i)
    ...
    ↔
    ∀ l, Finsupp.total ι M R v l = 0 → l = 0 --Finsupp.total ι M R v 传入参数l的意思
      := by
