...
--         done
      ...
      -- (α1: ℝ )
          ...
          -- (range :Finset n)
          -- :
          -- mulVec A x = fun yi ↦ ∑ xi in range, x xi • A yi xi
          --   := by
          --   simp only [mulVec]
          --   ext h7_x
          --   rw [dotProduct]
          ...
          --   · exact fun x_1 a ↦ mul_comm (A h7_x x_1) (x x_1)
              ...
              (A : Matrix m n R)
              (x : Matrix n Unit R)
              : A * x = (fun xi => fun yi => ∑ zi, (A xi zi) • (x zi yi) )
                := by
                simp only [smul_eq_mul]
                ext x1 x2
                exact rfl
          ...
          {x : Matrix n Unit R}
          {b : Matrix m Unit R}
          (hA : ∀ (b : Matrix m Unit R), ∃! (x: Matrix n Unit R), A * x = b)
          : LinearIndependent R (fun i ↦ Aᵀ i)
            ...
            simp only at h3
            clear hA
            have h6_oppo : ∃ (x: Matrix n Unit R) (y: Matrix n Unit R)
            , (x≠y) ∧ (A * x = 0) ∧ (A * y = 0)
              := by
              use 0
              use (Matrix.col h2) -- n → Fin 1 → R
              constructor
              ...
              · have h10:= MainGoal4_1 A (col h2)
                rw [h10]
                clear h10

                have h9: (∑ x:n, h2 x • fun y ↦ A y x)
                ...
                simp only [col_apply, smul_eq_mul]
