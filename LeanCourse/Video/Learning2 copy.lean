...
[hp : Fact p.Prime]
(hxr : x ^ n = m)
(hv_1 :PartENat := (multiplicity (p : ℤ) m))
(hv_2 :Int.natAbs p ≠ 1 := hp.1.ne_one)
(hv_3 :=(finite_int_iff.2 hv_2 hm))
-- (hv2 )hp.1.ne_one):Int.natAbs a ≠ 1 ∧ b ≠ 0 := finite_int_iff.2  )
(hv : Part.get hv_1 (hv_3) % n ≠ 0)
