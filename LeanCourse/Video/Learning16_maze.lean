...
structure GameState where
  size     : Coords      -- 右下角的地图块 coordinates of bottom-right cell
  position : Coords      -- 玩家的位置 row and column of the player
  walls    : List Coords -- 一个列表的墙 maze cells that are not traversible
...
-- instance : ToString CellContents where
--   toString : CellContents → String :=
--     fun c =>
--       if c = CellContents.empty then "empty"
--       else if c = U then "U"
--       else if c = D then "D"
--       else if c = R then "R"
--       else s!"{repr c}"
